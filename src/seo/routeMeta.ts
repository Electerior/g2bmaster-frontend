...
export interface RouteMeta {
   ...
   *
   * 열다섯 주소에 카드를 열다섯 장 만들지 않는다. 이유는 아래 [ROUTES.beta] 에 적었다.
   */
  image?: OgImage;
}
...
export const ROUTE_META: Readonly<Record<RoutePath, RouteMeta>> = {
  ...
  [ROUTES.beta]: {
     ...
     * routes"였지만, 본문이 실제로 요구한 것은 열다섯 장이 아니라 한 장이다 — "`/beta` is
     * the only page anyone will share, and it has no card of its own."
     *
     * 나머지 열넷을 만들지 않은 이유는 라우트마다 다르다.
     *
     *  - /saved · /company · /officers · /analysis-lab · /system — 위에서 noindex 다.
     ...
     *  - /spec-search · /analysis-lab · /system — 화면 속이 아직 '준비 중'이다. 카드는
     *    페이지가 주는 것을 약속하는 그림인데, 여기서 약속할 것이 아직 없다.
     *  - 나머지 앱 화면(/notices/bid-result · /deal-radar · /price-db · /trends/*) — 검색
     *    결과나 필터 상태를 보는 도구이고, 카카오톡으로 남에게 붙여 넣는 대상이 아니다.
     *    공유가 실제로 일어나는 자리에만 카드를 만든다. 열다섯 장을 그려 두면 랜딩 문구가
     *    바뀔 때마다 열다섯 장을 다시 그려야 하고, 그리지 않으면 열넷이 조용히 낡는다.
     *
     * 카드가 필요해지면 이 표에 image 한 줄을 더하고 scripts/og-cards.py 의 CARDS 에
     * 함수를 하나 더한다. 배선은 이미 라우트별로 돌아간다.
     */
    image: BETA_CARD,
  },
};
