...
/**
 * 접수처 — 구글 Apps Script 웹앱(docs/beta-signup.gs).
 *
 * 주소를 .env 가 아니라 코드에 둔다. 서버는 이 저장소를 받아 그대로 띄우므로, .env 를
 * 따로 두게 하면 배포마다 한 단계가 늘고 그 한 단계를 빠뜨리면 접수가 조용히 Spring 쪽
 * (아직 배포돼 있지 않다)으로 넘어가 폼이 통째로 실패한다. 실제로 그렇게 새어 나갔다.
 *
 * 공개 저장소에 남는 것은 감수한다. VITE_ 접두 변수는 빌드 때 번들에 그대로 박히므로
 * 랜딩을 여는 누구나 개발자 도구에서 같은 주소를 읽는다 — 저장소에 두지 않아도 이미
 * 공개된 값이다. 반대로 시트 문서 ID 는 여기 없다. 그것은 Apps Script 의 스크립트 속성
 * 에만 있고 클라이언트로 내려가지 않는다.
 *
 * 환경변수를 주면 그쪽이 이긴다. 빈 문자열을 명시하면(VITE_BETA_SHEET_URL=) Spring 의
 * /api/beta/* 로 돌아간다 — 백엔드 접수가 배포되는 날의 전환 손잡이다.
 */
const DEFAULT_SHEET_URL =
  'https://script.google.com/macros/s/AKfycbxEqfpAqAi25RbUtf-tqjLAXomi6aU84MNKtSeolJLg0AIjTNmWDjYcGl7c4TPd3Cse/exec';

const SHEET_URL = import.meta.env.VITE_BETA_SHEET_URL ?? DEFAULT_SHEET_URL;
