...
export function IndexNoticeDrawer({ seed, onClose }: IndexNoticeDrawerProps) {
  ...
  const rows = buildMetaRows([
    ['공고번호', item.noticeOrder ? `${item.id} (${item.noticeOrder}차)` : String(item.id)],
    ['단계', item.category ?? ''],
    ['상태', item.state ?? ''],
    ['공고기관', primary],
    ...
    ['품명', products.length ? products.map((p) => p.name).filter(Boolean).join(', ') : ''],
  ...
  return (
    <Drawer open onClose={onClose} label={title}>
      ...
      <DrawerMeta rows={rows}>
        {products.length ? (
          <Collapsible summary={`물품목록 ${products.length}건`}>
            {/* .raw-grid 는 2열 격자다 — 키와 값이 격자의 직계 자식이어야 줄이 맞는다. */}
