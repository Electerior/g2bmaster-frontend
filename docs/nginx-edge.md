...
2. **정해지면 `listen` 을 거기에 맞춘다.** 지금 이 파일은 8001, certbot vhost 는 443,
   ...
   **`listen` 만 정하고 끝내지 마라 — `/api/` 와 `/healthz` 의 `proxy_pass` 업스트림도 함께
   정해야 한다.** 이 둘은 오래 어긋나 있었다. 설정은 `127.0.0.1:8080` 을 'Spring Boot' 라고
   부르고 있었는데, 같은 파일 머리말은 이미 8080 이 cloudflared 터널 입구(= serve-static.mjs)
   라고 측정 결과를 적어 두고 있었다. 감사가 머리말만 갱신하고 업스트림을 남긴 표류다.
   실제 백엔드는 8082 이고(serve-static.mjs 의 `BACKEND` 기본값), 지금은 그 값으로 고쳤다.
   어긋난 채 엣지를 옮기면 화면은 200 인데 API 만 전부 502 가 된다 — 가장 늦게 발견되는 종류다.
