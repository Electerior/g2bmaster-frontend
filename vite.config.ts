...
const config: ViteConfigWithTest = {
  ...
  build: {
     ...
     *
     * **이 값은 한 번 되살아난 적이 있다.** PR #16 이 false 로 바꿨는데 PR #24(프리렌더)를
     * 머지하며 vite.config.ts 충돌을 브랜치 쪽으로 풀어 true 가 돌아왔다. 사람 눈에는
     * 아무 증상이 없고 dist 를 열어 봐야만 보이는 회귀라, 아래 vite.config.test.ts 가 잠근다.
     */
    sourcemap: false,
    /**
     ...
     * (바로 아래 manualChunks)에 따라 조용히 틀린 파일을 링크하게 된다.
    ...
    rollupOptions: {
      output: {
         ...
         *
         * 소스맵과 같은 경위로 한 번 사라졌던 배선이다 — vendorChunk 함수는 파일에 남아
         * 있는데 여기 연결만 빠져서, 함수를 읽은 사람은 벤더가 갈리고 있다고 믿게 된다.
         * 죽은 코드가 아니라 **거짓말하는 코드**였다. 아래 테스트가 이 연결을 잠근다.
         */
