...
const config: ViteConfigWithTest = {
  ...
  build: {
     ...
     * (바로 아래 manualChunks)에 따라 조용히 틀린 파일을 링크하게 된다. manifest 는
     * "이 모듈이 어느 파일이 됐고 무엇을 함께 필요로 하는가"를 vite 가 직접 적어 주는
     * 유일한 출처다.
     *
     ...
     *
     * 이 브랜치가 벤더를 가를 때 남긴 "앱 청크가 160KB 대인 건 라우터가 화면 14개를 전부 정적
     * import 하기 때문이고 라우트 코드 스플릿은 별도 브랜치 예정"이라는 메모는 **이 통합에서
     * 해소됐다** — perf/route-code-split 이 여기 함께 들어와 진입 번들이 593kB → 323kB 가 됐다.
     * 그 브랜치 말대로 이 파일은 손댈 필요가 없었다. 동적 import 는 rollup 이 알아서 가른다.
     */
  },
  test: {
    environment: 'jsdom',
